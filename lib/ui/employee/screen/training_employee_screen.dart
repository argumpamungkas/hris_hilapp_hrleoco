...
class TrainingEmployeeScreen extends StatelessWidget {
  const TrainingEmployeeScreen({super.key});
            ...
            children: [
              ...
              Consumer<EmployeeProvider>(
                builder: (context, prov, _) {
                  return HeaderEmployeeAction(
                    ...
                    onTap: () {
                      Navigator.pushNamed(context, Routes.addTrainingScreen).then((value) {
                        prov.fetchEmployee();
                      });
                    },
                  );
              ...
              Expanded(
                child: Consumer<EmployeeProvider>(
                  builder: (context, prov, _) {
                    if (prov.employeeResponseModel!.employeeTrainings.isEmpty) {
                      return Center(child: Text("Data Training is Empty"));
                    }
                    ...
                    return RefreshIndicator(
                      ...
                      child: ListView.builder(
                        itemCount: prov.employeeResponseModel!.employeeTrainings.length,
                        itemBuilder: (context, index) {
                          final employee = prov.employeeResponseModel!.employeeTrainings[index];
                          return Card(
                            child: ExpansionTile(
                              ...
                              children: [
                                Divider(),
                                ItemDataUser(title: "Training Name", value: employee.name ?? "-"),
                                ItemDataUser(title: "Description", value: employee.description ?? "-"),
                                ItemDataUser(title: "Date", value: employee.start ?? "-"),
                                ItemDataUser(title: "Profession", value: employee.profesion ?? "-"),
                                ...
                                ElevatedButton.icon(
                                  onPressed: () {
                                    showConfirmDialog(
                                      ...
                                      descriptionConfirm: "Are you sure remove data ${employee.name} ?",
                                      action: [
                                        TextButton(
                                          onPressed: () {
                                        ...
                                        TextButton(
                                          onPressed: () async {
                                            ...
                                            final result = await prov.deleteEmployee(employee.id!, "deleteTraining");
