...
class ChangeDaysNotificationWidget extends StatelessWidget {
  const ChangeDaysNotificationWidget({super.key, required this.dataItem, required this.prov, required this.module, required this.dataNotif});

  final List<ResultNotificationDetailChangeDays> dataItem;
  ...
  Widget build(BuildContext context) {
    return Consumer2<NotificationProvider, ChangeDaysProvider>(
      builder: (context, notifC, changeDaysC, _) {
            ...
            Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ResultNotificationDetailChangeDays item = dataItem[index];
                  if (item.start != null) {
                    var dateFormatDefautlt = DateFormat("yyyy-MM-dd").parse(item.start!);
                    dayFrom = formatCreated(dateFormatDefautlt);
                  }

                  if (item.end != null) {
                    var dateFormatDefautlt = DateFormat("yyyy-MM-dd").parse(item.end!);
                      ...
                      children: [
                        ...
                        Column(
                          children: [
                            ...
                            ItemDataNotification(title: "Start", value: dayFrom!),
                            ItemDataNotification(title: "Replace To", value: replaceTo!),
                            ItemDataNotification(title: "Remarks", value: item.remarks ?? "-"),
                          ],
                        ),
                                                  ...
                                                  onPressed: () async {
                                                    ...
                                                    await prov.approved(dataNotif, item.id, module).then((value) {
                                                      ...
                                                      if (value.theme == "success") {
                                                        ...
                                                        changeDaysC.fetchChangeDays(context);
                                                        Navigator.pop(context);
